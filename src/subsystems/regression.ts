...
function applyLayout(state: RegressionState): void {
  ...
  [state.undoButton, state.redoButton, state.drawButton, state.eraseButton, state.toolsButton].forEach((button) => {
    button.style.width = dgsButtonSize;
    button.style.height = dgsButtonSize;
    button.style.minWidth = dgsButtonSize;
    button.style.minHeight = dgsButtonSize;
    button.style.borderRadius = usesDgsLayout ? '8.75px' : '';
    button.style.borderWidth = usesDgsLayout ? '2.5px' : '';
    const icon = button.querySelector<SVGElement>('svg');
    if (icon) {
      icon.style.width = dgsIconSize;
      icon.style.height = dgsIconSize;
    }
  });
  ...
  state.undoButton.style.top = usesDgsLayout ? (dgsMenuOpen ? '58px' : '50px') : 'auto';
  ...
  state.undoButton.style.transition = usesDgsLayout ? 'top 220ms cubic-bezier(.2, .8, .2, 1)' : '';

  state.redoButton.style.position = 'absolute';
  state.redoButton.style.left = usesDgsLayout ? '10px' : '46px';
  state.redoButton.style.top = usesDgsLayout ? (dgsMenuOpen ? '101px' : '93px') : 'auto';
  ...
  state.drawButton.style.left = usesDgsLayout ? '466px' : '82px';
  ...
  state.eraseButton.style.left = usesDgsLayout ? '509px' : '118px';
  ...
  state.toolsButton.style.left = usesDgsLayout ? '552px' : '154px';
  ...
  state.drawColorMenu.style.left = usesDgsLayout ? Math.min(466, popupMaxLeft) + 'px' : '10px';
  ...
  state.toolsMenu.style.left = usesDgsLayout ? Math.min(552, popupMaxLeft) + 'px' : '184px';
...
function requestRegressionDgs(
  ...
  // Only authored @Regression anchors (including the anchor generated by
  // @Reconstruction) request the compact DGS shell. The regression instance
  // created automatically by @DGS has no anchor and must not recurse.
  if (!anchor || typeof window.__setupDGSRegression !== 'function') return;
  try { window.__setupDGSRegression(uid, boardId, language); } catch (e) {}
}
...
function setupRegressionUI(uid: string, boardId: string, languageCode?: string): void {
  ...
  const boardInstance = Object.keys(states)
  ...
  if (boardInstance && boardInstance !== states[uid]) {
    ...
    requestRegressionDgs(uid, boardId, language, anchor);
    return;
  }
