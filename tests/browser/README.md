The full Schar, localization, and quiz-family reports below belong to functional checkpoint `65d6e54` (bundle `583d1e77...`). The later restoration of the original overlay design has its own focused before/after verification at the end of this document.
...
The original overlay design was then restored and checked against the actual unmodified `8dcdbb50` browser bundle, both in the standalone `<jsx-graph>` ShadowRoot and in the real LiaScript presentation:

```text
node tests/scharDesign.browser.mjs --before
node tests/scharDesign.browser.mjs
```

[schar-design-before.json](schar-design-before.json) and [schar-design-after.json](schar-design-after.json) record matching original dimensions and computed visual styles for the panel, native range, parameter row/label, term toggle, minimize button, and resize handle. The original scale is 0.55: a 190px-wide panel appears 104.5px wide, and the native range remains 129 by 12 CSS pixels (70.95 by 6.6 rendered pixels). Padding, fonts, colors, border, shadow, margins, and transforms match the original. The preserved lower position beneath the DGS controls and the width of translated term text are excluded from equality checks. The original ShadowRoot appearance has no visible round thumb; the later enlarged thumb and newly applied visual rules are removed.

Both focused scenarios passed on bundle SHA-256 `c122c1b35c096938350a613aa6158a0fb26a0afc19c232adbc23bb92c85f6523`: native range dragging produced 12 input events with stable input identity and viewport, native minimize/restore worked, and dragging the parabola right by 1 and down by 2 produced a=0.5, b approximately 1, c approximately -2 and accepted reconstruction. The existing interaction fixture's range dimensions and pointer targeting were adjusted to the restored design. TypeScript checking, all 220 unit tests, and both builds also passed; the full 36-scenario and seven-family quiz browser suites were not repeated for this visual-only restoration.

Visual comparison: [original standalone overlay](schar-design-before-board.png), [restored standalone overlay](schar-design-after-board.png), [original LiaScript overlay](schar-design-before-viewer.png), [restored LiaScript overlay](schar-design-after-viewer.png).
